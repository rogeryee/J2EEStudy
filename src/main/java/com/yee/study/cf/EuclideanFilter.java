package com.yee.study.cf;

import java.util.*;

/**
 * 本例中使用了欧几里德距离算法完成协同过滤的用户推荐
 *
 * Author: RogerYee
 */
public class EuclideanFilter
{
    ...
    static {
        for (String str : EuclideanFilter.users) {
            userSet.add(str);
        }
        for (String str : EuclideanFilter.films) {
            filmSet.add(str);
        }
        score = getScore();
    }
    ...
    private static Map getScore() {
        ...
        score.put(EuclideanFilter.users[0], tempScore);
        // bbb
        ...
        score.put(EuclideanFilter.users[1], tempScore);
        // ccc
        tempScore = new HashMap();
        tempScore.put(films[0], 9);
        tempScore.put(films[1], 9);
        ...
        score.put(EuclideanFilter.users[2], tempScore);
        // ddd
        tempScore = new HashMap();
        ...
        score.put(EuclideanFilter.users[3], tempScore);
        // 葛二蛋
        tempScore = new HashMap();
        ...
        score.put(EuclideanFilter.users[4], tempScore);
        return score;
    }

    public static void main(String[] args) {
        //
        System.out.println(EuclideanFilter.users[0] + " 与其他人的相似度（分值越低越相似）：");
        EuclideanFilter.outNearbyUserList(EuclideanFilter.users[0]);
    }
}
